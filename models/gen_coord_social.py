...
class GenCoordModel_Social():
    ...
    def train(self, data, train_size, entities=[0,1]):
        ...
        final_reg = None
        cur_loss = 1e8
        loss_list = [mean_squared_error(f_gt, np.zeros(f_gt.shape))]
        for iter_id in range(10):
            ...
            for gen_coord_id in gen_coord_list:
                ...
                if types[gen_coord_id] == 'ref_point' and \
                   types[gen_coord_id] in selected_types: continue 
                ...
                # predict
                f_pred_list = [self.pred_forces(reg, mask, gen_coords[episode_id], cur_selected, entities=entities)
                                for episode_id in range(num_episodes)]
            ...
            # vel_pred_list = [self.pred_vels(final_reg, final_mask, gen_coords[episode_id], selected, vels[episode_id], entities=entities)
            #                     for episode_id in range(num_episodes)]
            ...
            # #     if np.linalg.norm(A[i,:]) < 0.2 and np.linalg.norm(B[i,:]) < 0.2:
            # #         A[i,:] = [1, 1]
            # #         B[i,:] = [1, 1]
            ...
            #                     for episode_id in range(num_episodes)]
            # f_pred = np.concatenate([f.reshape(-1, 2) for f in f_pred_list], 0)
            # # print(np.linalg.norm(f_gt, axis=1).mean())
            # A, B = f_gt.reshape(-1, 2).copy(), f_pred.reshape(-1, 2).copy()
            # # print([np.concatenate([a, b, c], axis=-1) for a, b, c in zip(vel_gt_list_noinit[0], f_gt_list[0], f_pred_list[0])])
            # # print(np.concatenate([A, B], axis=1))
            # for i in range(A.shape[0]):
            #     if np.linalg.norm(A[i,:]) < 0.2 * MASS and np.linalg.norm(B[i,:]) < 0.2 * MASS:
            #         A[i,:] = [1, 1]
            #         B[i,:] = [1, 1]
            # cos_AB = ((A * B).sum(1) / (np.linalg.norm(A, axis=1) * np.linalg.norm(B, axis=1)))
            ...
            # # print(np.concatenate([A, B], axis=1))
            # for i in range(A.shape[0]):
            #     if np.linalg.norm(A[i,:]) < 0.3 and np.linalg.norm(B[i,:]) < 0.1:
            #         A[i,:] = [1, 1]
            #         B[i,:] = [1, 1]
            # cos_AB = ((A * B).sum(1) / (np.linalg.norm(A, axis=1) * np.linalg.norm(B, axis=1)))
        ...
        if self.model_path:
            pickle.dump({'selected': selected, 'types': selected_types, 
                         'reg': final_reg, 'mask': final_mask, 'loss_list': loss_list,
